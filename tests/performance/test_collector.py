...
class TestCollector:
    ...
    def setup_class(cls):
        ...
        with open("output.csv", "w") as out_file:
            out_file.writelines(
                "data rate [Mbps], real data rate [Mbps], freq [Hz], real freq [Hz], success rate [%], # updates received,  # updates expected, # PVs, PV length, # cycles, # triggers, test time [s] \n"
            )
    ...
    async def configure_ioc(self, n_cycles=None, pv_len=None, n_pvs=None):
        ...
        if n_cycles is not None:
            await self.ctxt.put("SDS:TEST:N_CYCLES", n_cycles)
    ...
    async def single_test(
        self, n_cycles: int, n_pvs: int, pv_len: int, freq: float, n_triggers: int
    ):
        first_cycle, last_cycle, timestamp = await self.trigger_ioc_updates(
            n_cycles, n_pvs, pv_len, freq, n_triggers
        )
        ...
        pv_updates_collected, elapsed = await self.check_output(
            n_cycles, directory, first_cycle, n_triggers
        )
    ...
    async def trigger_ioc_updates(
        ...
        last_cycle = first_cycle + n_cycles - 1
        for i in range(n_triggers):
            t0 = time.time()
            mon, queue = await self.wait_for_pv_value(last_cycle + i * n_cycles)
            await self.trigger()
            value = await asyncio.wait_for(queue.get(), 5)
            if mon is not None:
                mon.close()
            assert value[0] == last_cycle + i * n_cycles
            mon.close()
            time_delta = n_cycles / freq - (time.time() - t0)
            if time_delta > 0:
                await asyncio.sleep(time_delta)
            else:
                print(f"time_delta={time_delta}")
        return first_cycle, last_cycle, value.timestamp

    async def check_output(
        self, n_cycles: int, directory: Path, first_cycle: int, n_triggers: int
    ):
        # Check files
            ...
            for i in range(n_triggers):
                try:
                    file_path = (
                        ...
                        / directory
                        / (collector.name + f"_{int(first_cycle + i * n_cycles)}.h5")
                    )
                    ...
                    trigger = entry.entries.get(
                        f"sds_event_{int(first_cycle + i * n_cycles)}"
                    )
                    ...
                    for n in range(n_cycles):
                        try:
                            cycle = trigger.entries.get(
                                f"cycle_{int(first_cycle + i * n_cycles)+n}"
                            )
                            assert cycle is not None

                            for pv in collector.pvs:
                                if pv in pv_list:
                                    pv_field = cycle.entries.get(pv)
    ...
    async def test_parametric(
        ...
        await self.configure_ioc(n_cycles=n_cycles, pv_len=pv_len, n_pvs=n_pvs)
        ...
        n_triggers = int(n_triggers / n_cycles)
        ...
        real_freq = n_cycles * (n_triggers - 1) / elapsed
        data_rate_real = n_pvs * pv_len * real_freq * 1e-6 * 8 * 8
        success_rate = pv_updates_collected / (n_pvs * n_cycles * n_triggers)
        with open("output.csv", "a") as out_file:
            out_file.writelines(
                f"{data_rate}, {data_rate_real:.3f}, {freq}, {real_freq:.3f}, {success_rate*100:.3f}, {pv_updates_collected}, {n_pvs * n_cycles * n_triggers}, {n_pvs}, {pv_len}, {n_cycles},{n_triggers}, {elapsed:.3f}\n"
            )
