...
indexer = elastic.IndexerClient()


@app.on_event("startup")
async def startup_event():
    indexer.wait_for_connection(RETRY_CONNECTION)
    indexer.check_indices()
...
async def get_collector(name: str, method_name: str, method_type: int,
    ...
    id = await indexer.get_collector_id(name, method_name, method_type,
                                        pv_list)
